<?php
/**
 * Class for the customizer importer used in the Academy Starter Templates plugin.
 *
	...
	public static function import( $customizer_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing customizer settings', 'academy-starter-templates' )
			);
		} else {
			// Add this message to log file.
			$log_added = Helpers::append_to_file(
				esc_html__( 'Customizer settings import finished!', 'academy-starter-templates' ),
				$log_file_path,
				esc_html__( 'Importing customizer settings', 'academy-starter-templates' )
			);
		}//end if
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %s - file path */
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'academy-starter-templates' ),
					$import_file_path
				)
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'academy-starter-templates' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'academy-starter-templates' )
			);
		}

		// Import images.
		if ( Helpers::apply_filters( 'academyst/customizer_import_images', true ) ) {
		...
		$use_wp_customize_save_hooks = Helpers::apply_filters( 'academyst/enable_wp_customize_save_hooks', false );
