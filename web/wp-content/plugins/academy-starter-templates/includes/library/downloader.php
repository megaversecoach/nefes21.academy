...
class Downloader {
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url',
				__( 'Missing URL for downloading a file!', 'academy-starter-templates' )
			);
		}
		...
		$response = wp_remote_get(
			$url,
			array( 'timeout' => Helpers::apply_filters( 'academyst/timeout_for_downloading_import_file', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				...
				sprintf( /* translators: %1$s and %3$s - strong HTML tags, %2$s - file URL, %4$s - br HTML tag, %5$s - error code, %6$s - error message. */
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'academy-starter-templates' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
				...
				Helpers::apply_filters( 'academyst/message_after_file_fetching_error', '' )
			);
		}
	...
	public function set_download_directory_path( $download_directory_path ) {
		if ( file_exists( $download_directory_path ) ) {
			...
			$this->download_directory_path = Helpers::apply_filters( 'academyst/upload_file_path', trailingslashit( $upload_dir['path'] ) );
		}
	}
}
