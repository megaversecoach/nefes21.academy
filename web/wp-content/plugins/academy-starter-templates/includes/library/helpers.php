...
class Helpers {
	...
	public static function download_import_files( $import_file_info ) {
		...
		$import_file_info = self::apply_filters( 'academyst/pre_download_import_files', $import_file_info );
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			...
			$content_filename = self::apply_filters( 'academyst/downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'academyst/downloaded_content_file_suffix_and_file_extension', '.xml' );
		...
		if ( ! empty( $import_file_info['import_widget_file_url'] ) ) {
			...
			$widget_filename = self::apply_filters( 'academyst/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'academyst/downloaded_widgets_file_suffix_and_file_extension', '.json' );
		...
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) ) {
			...
			$customizer_filename = self::apply_filters( 'academyst/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'academyst/downloaded_customizer_file_suffix_and_file_extension', '.dat' );
		...
		if ( ! empty( $import_file_info['import_redux'] ) && is_array( $import_file_info['import_redux'] ) ) {
			...
			foreach ( $import_file_info['import_redux'] as $index => $redux_item ) {
				$redux_filename = self::apply_filters( 'academyst/downloaded_redux_file_prefix', 'demo-redux-import-file_' ) . $index . '-' . self::$demo_import_start_time . self::apply_filters( 'academyst/downloaded_redux_file_suffix_and_file_extension', '.json' );
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %1$s - br HTML tag, %2$s - file path */
					__( 'An error occurred while writing file to your server! Tried to write a file to: %1$s%2$s.', 'academy-starter-templates' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %1$s - br HTML tag, %2$s - file path */
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %1$s%2$s.', 'academy-starter-templates' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %1$s and %2$s - strong HTML tags, %3$s - HTML link to a doc page. */
					__( 'This WordPress page does not have %1$sdirect%2$s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %3$s.', 'academy-starter-templates' ),
					'<strong>',
					'</strong>',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'academy-starter-templates' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'academy-starter-templates' )
			);
		}
	...
	public static function get_log_path() {
		...
		$upload_path = self::apply_filters( 'academyst/upload_file_path', trailingslashit( $upload_dir['path'] ) );

		$log_path = $upload_path . self::apply_filters( 'academyst/log_file_prefix', 'log_file_' ) . self::$demo_import_start_time . self::apply_filters( 'academyst/log_file_suffix_and_file_extension', '.txt' );
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$filetype  = wp_check_filetype( basename( $log_path ), self::apply_filters( 'academyst/file_mimes', $log_mimes ) );

		// Prepare an array of post data for the attachment.
		$attachment = array(
			'guid'           => self::get_log_url( $log_path ),
			'post_mime_type' => $filetype['type'],
			'post_title'     => self::apply_filters( 'academyst/attachment_prefix', esc_html__( 'Academy Starter Templates - ', 'academy-starter-templates' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function get_log_url( $log_path ) {
		$upload_dir = wp_upload_dir();
		$upload_url = self::apply_filters( 'academyst/upload_file_url', trailingslashit( $upload_dir['url'] ) );
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf( /* translators: %1$s - opening div and paragraph HTML tags, %2$s - closing div and paragraph HTML tags. */
					__( '%1$sYour user role isn\'t high enough. You don\'t have permission to import demo data.%2$s', 'academy-starter-templates' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$file_not_provided_error = array(
			'error' => esc_html__( 'No file provided.', 'academy-starter-templates' ),
		);
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Content file was not uploaded. Error: %s', 'academy-starter-templates' ),
					$content_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files', 'academy-starter-templates' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Widget file was not uploaded. Error: %s', 'academy-starter-templates' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Customizer file was not uploaded. Error: %s', 'academy-starter-templates' ),
		...
		if ( $redux_file_info && ! isset( $redux_file_info['error'] ) ) {
			if ( isset( $_POST['redux_option_name'] ) && empty( $_POST['redux_option_name'] ) ) {
				...
				self::log_error_and_send_ajax_response(
					esc_html__( 'Missing Redux option name! Please also enter the Redux option name!', 'academy-starter-templates' ),
					$log_file_path,
					esc_html__( 'Upload files', 'academy-starter-templates' )
				);
			}
			...
			$log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Redux file was not uploaded. Error: %s', 'academy-starter-templates' ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'academy-starter-templates' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', 'academy-starter-templates' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		sprintf( /* translators: %s - the max execution time. */
			__( 'Initial max execution time = %s', 'academy-starter-templates' ),
		...
		sprintf( /* translators: %1$s - new line break, %2$s - the site URL, %3$s - the file path for content import, %4$s - the file path for widgets import, %5$s - the file path for widgets import, %6$s - the file path for redux import. */
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sRedux files:%1$s%6$s', 'academy-starter-templates' ),
			PHP_EOL,
			get_site_url(),
			empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'academy-starter-templates' ) : $selected_import_files['content'],
			empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'academy-starter-templates' ) : $selected_import_files['widgets'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'academy-starter-templates' ) : $selected_import_files['customizer'],
			empty( $redux_file_string ) ? esc_html__( 'not defined!', 'academy-starter-templates' ) : $redux_file_string
		);
	}
	...
	public static function set_demo_import_start_time() {
		self::$demo_import_start_time = date( self::apply_filters( 'academyst/date_format_for_file_names', 'Y-m-d__H-i-s' ) );
	}
	 ...
	 */
	public static function set_academyst_import_data_transient( $data ) {
		set_transient( 'academyst_importer_data', $data, 0.1 * HOUR_IN_SECONDS );
	}


	/**
	 * Backwards compatible apply_filters helper.
	 * With 3.0 we changed the filter prefix from 'pt-academyst/' to just 'academyst/',
	 ...
	 * With 3.0 we changed the action prefix from 'pt-academyst/' to just 'academyst/',
	 * but we needed to make sure backwards compatibility is in place.
	...
	public static function get_plugin_page_setup_data() {
		return self::apply_filters( 'academyst/plugin_page_setup', array(
			'parent_slug' => ACADEMY_STARTER_PLUGIN_SLUG,
			'page_title'  => esc_html__( 'Academy Starter', 'academy-starter-templates' ),
			'menu_title'  => esc_html__( 'Academy Starter', 'academy-starter-templates' ),
			'capability'  => 'import',
			'menu_slug'   => ACADEMY_STARTER_PLUGIN_SLUG,
		) );
	}
}
