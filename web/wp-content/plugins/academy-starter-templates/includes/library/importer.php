<?php
/**
 * Class for declaring the content importer used in the Academy Starter Templates plugin
 *
...
class Importer {
	...
	/**
	 * The instance of the Academy Starter Templates class.
	 *
	 * @var object
	 */
	private $academyst;
	...
	public function import_content( $import_file_path ) {
		...
		if ( strpos( ini_get( 'disable_functions' ), 'set_time_limit' ) === false ) {
			set_time_limit( Helpers::apply_filters( 'academyst/set_time_limit_for_demo_data_import', 300 ) );
		}
		...
		if ( ! Helpers::apply_filters( 'academyst/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > Helpers::apply_filters( 'academyst/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			if ( ! empty( $message ) ) {
				$this->academyst->append_to_frontend_error_messages( $message );
			}

			// Add message to log file.
			$log_added = Helpers::append_to_file(
				__( 'New AJAX call!', 'academy-starter-templates' ) . PHP_EOL . $message,
				$this->academyst->get_log_file_path(),
				''
			);
	...
	private function set_current_importer_data() {
		$data = array_merge( $this->academyst->get_current_importer_data(), $this->get_importer_data() );

		Helpers::set_academyst_import_data_transient( $data );
	}
}
