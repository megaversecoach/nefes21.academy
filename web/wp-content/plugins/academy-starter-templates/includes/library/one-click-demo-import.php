...
class OneClickDemoImport {
	...
	public function import_demo_data_ajax_callback() {
		// Set a higher memory limit for large imports.
		ini_set( 'memory_limit', Helpers::apply_filters( 'academyst/import_memory_limit', '350M' ) );
		// Set a higher execution time limit for large imports.
		ini_set( 'max_execution_time', Helpers::apply_filters( 'academyst/import_execution_time_limit', 600 ) );

		// Verify nonce and user capability.
		Helpers::verify_ajax_call();

		$user_existing_importer_data = $this->use_existing_importer_data();

		if( ! $user_existing_importer_data ) {
			Helpers::set_demo_import_start_time();
			...
			if( ! empty( $_FILES ) ) {
				...
				wp_send_json_error( esc_html__( 'No import files specified!', 'academy-starter-templates' ) );
			}
		...
		if ( ! $this->before_import_executed ) {
			...
			// Run any actions hooked to run before content import.
		...
		if ( ! empty( $this->selected_import_files['content'] ) ) {
			if( is_plugin_active('wordpress-importer/wordpress-importer.php') ) {
				Helper::get_wordpress_importer();
				$wp_import = new \WP_Import();
				$wp_import->fetch_attachments = true;
				ob_start();
				$wp_import->import( $this->selected_import_files['content'] );
				ob_get_clean();
			} else {
				wp_send_json_error( esc_html__( 'Content importer class not available.', 'academy-starter-templates' ) );
			}
		}
