...
class PluginInstaller {
	...
	public function init() {
		...
		add_action( 'academyst/plugin_intaller_before_plugin_activation', array( $this, 'before_plugin_activation' ) );
		add_action( 'academyst/plugin_intaller_after_plugin_activation', array( $this, 'after_plugin_activation' ) );

		add_action( 'wp_ajax_academyst_install_plugin', array( $this, 'install_plugin_callback' ) );
	}
	...
	public function get_partner_plugins() {
		return array(
			array(
				'name'        => esc_html__( 'WPForms', 'academy-starter-templates' ),
				'description' => esc_html__( 'Join 4,000,000+ professionals who build smarter forms and surveys with WPForms.', 'academy-starter-templates' ),
				'slug'        => 'wpforms-lite',
				'required'    => false,
				'preselected' => true,
			),
			array(
				'name'        => esc_html__( 'All in One SEO', 'academy-starter-templates' ),
				'description' => esc_html__( 'Use All in One SEO Pack to optimize your WordPress site for SEO.', 'academy-starter-templates' ),
				'slug'        => 'all-in-one-seo-pack',
				'required'    => false,
				'preselected' => true,
			),
			array(
				'name'        => esc_html__( 'MonsterInsights', 'academy-starter-templates' ),
				'description' => esc_html__( 'The #1 Google Analytics Plugin for WordPress that’s easy and powerful.', 'academy-starter-templates' ),
				'slug'        => 'google-analytics-for-wordpress',
				'required'    => false,
				'preselected' => true,
			),
			array(
				'name'        => esc_html__( 'Custom Landing Pages by SeedProd', 'academy-starter-templates' ),
				'description' => esc_html__( 'Work on your site in private while visitors see a "Coming Soon" or "Maintenance Mode" page.', 'academy-starter-templates' ),
				'slug'        => 'coming-soon',
				'required'    => false,
			),
			array(
				'name'        => esc_html__( 'Smash Balloon Social Photo Feed', 'academy-starter-templates' ),
				'description' => esc_html__( 'Display beautifully clean, customizable, and responsive Instagram feeds.', 'academy-starter-templates' ),
				'slug'        => 'instagram-feed',
				'required'    => false,
			),
			array(
				'name'        => esc_html__( 'WP Mail SMTP', 'academy-starter-templates' ),
				'description' => esc_html__( 'Make email delivery easy for WordPress. Connect with SMTP, Gmail, Outlook, Mailgun, and more.', 'academy-starter-templates' ),
				'slug'        => 'wp-mail-smtp',
				'required'    => false,
			),
		);
	...
	public function set_plugins() {
		$all_plugins = Helpers::apply_filters( 'academyst/register_plugins', array() );
	...
	public function get_theme_plugins() {
		$theme_plugins = Helpers::apply_filters( 'academyst/register_plugins', array() );
	...
	public function install_plugin_callback() {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( esc_html__( 'Could not install the plugin. You don\'t have permission to install plugins.', 'academy-starter-templates' ) );
		}

		$slug = ! empty( $_POST['slug'] ) ? sanitize_key( wp_unslash( $_POST['slug'] ) ) : '';

		if ( empty( $slug ) ) {
			wp_send_json_error( esc_html__( 'Could not install the plugin. Plugin slug is missing.', 'academy-starter-templates' ) );
		}

		// Check if the plugin is already installed and activated.
		if ( $this->is_plugin_active( $slug ) ) {
			wp_send_json_success( esc_html__( 'Plugin is already installed and activated!', 'academy-starter-templates' ) );
		}
		...
		if ( $this->is_plugin_installed( $slug ) ) {
			...
			if ( ! is_wp_error( $activated ) ) {
				wp_send_json_success( esc_html__( 'Plugin was already installed! We activated it for you.', 'academy-starter-templates' ) );
			} else {
		...
		if ( ! $this->filesystem_permissions_allowed() ) {
			wp_send_json_error( esc_html__( 'Could not install the plugin. Don\'t have file permission.', 'academy-starter-templates' ) );
		}
		...
		if ( $upgrader->plugin_info() ) {
			...
			if ( ! is_wp_error( $activated ) ) {
				wp_send_json_success(
					esc_html__( 'Plugin installed and activated succesfully.', 'academy-starter-templates' )
				);
			} else {
		...
		wp_send_json_error( esc_html__( 'Could not install the plugin. WP Plugin installer could not retrieve plugin information.', 'academy-starter-templates' ) );
	}
	...
	private function activate_plugin( $plugin_filename, $slug ) {
		Helpers::do_action( 'academyst/plugin_intaller_before_plugin_activation', $slug );

		$activated = activate_plugin( $plugin_filename,'',false, true );

		Helpers::do_action( 'academyst/plugin_intaller_after_plugin_activation', $slug );
	...
	private function filesystem_permissions_allowed() {
		...
		$url   = esc_url_raw( $academyst->get_plugin_settings_url() );
	...
	protected function get_plugins_api( $slug ) {
		...
		if ( ! isset( $api[ $slug ] ) ) {
			...
			$api[ $slug ] = plugins_api( 'plugin_information', array(
				'slug' => $slug,
