...
<div class="container">
    ...
    $read_more     = get_theme_mod( 'education_zone_blog_section_readmore' , __( 'Read More', 'university-zone' ));
					...
					<li>
						<article class="post">
							<header class="entry-header">
                                ...
                                if( $ed_date ){ ?>
                                    <div class="entry-meta">
    									<span><i class="fa fa-calendar-o"></i><a href="<?php the_permalink(); ?>"><?php echo esc_html( get_the_date( __( 'j M, Y', 'university-zone' ) ) ); ?></a></span>
    								</div>
                                <?php } ?>
							</header>
						</article>
    ...
    <?php 
        $post_page = get_option( 'page_for_posts' ); 
            if ( $post_page && $post_page > 0 ) {
                 echo '<a href="'. esc_url( get_permalink( $post_page ) ) .'" class="btn-learn-more">'. esc_html( 'View All Blog', 'university-zone' ) .'</a>';
            }
        ?>
</div>
